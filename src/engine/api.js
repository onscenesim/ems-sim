...
// SPLIT TOKENS: 
// Fast turns get a lower ceiling to prevent runaway generation.
// Debriefs retain the large context window for comprehensive feedback.
  ...
  try {
    const response = await generateContentWithRetry({
      ...
      config: {
        ...
        thinkingConfig: { thinkingBudget: 0 } // Bypasses internal reasoning delay for instant turns
      }
    });
  ...
  try {
     const response = await generateContentWithRetry({
      ...
      config: {
        ...
        topP: 0.8,
        safetySettings: SAFETY_SETTINGS,
        thinkingConfig: { thinkingBudget: 0 } // Keeps analysis fast enough to avoid Render connection drops
      }
    });
