...
function assembleSeedBlock(seed) {
  ...
  lines.push('2. DICE ROLLS: All dice rolls are performed server-side and injected into the conversation as [SYSTEM ROLL: procedure — d20=X vs DC Y — OUTCOME]. Your ONLY job is to narrate the clinical consequence of that pre-determined outcome. NEVER write your own [ROLL: ...] notation. NEVER invent a dice result. NEVER decide success or failure yourself. If a user performs a procedure and no SYSTEM ROLL appears in the message, narrate it as a deterministic/no-roll action without any roll notation.');
