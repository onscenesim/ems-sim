...
function assembleSeedBlock(seed) {
  ...
  if (isCurveball) {
    ...
    lines.push(`PRIVATE CASE KEY — never reveal, hint at, suggest, or act on (see Rule 8): ${seed.hint || '(none — portray the presentation faithfully)'}`);
  } else {
  ...
  lines.push('VASCULAR ACCESS CONTINUITY: Track every line for the whole call. A line that failed, blew, or infiltrated is GONE — never push another drug through it and never let it quietly work again later. When the provider orders a medication without naming a route, use the best PATENT access the patient actually has (patent line before marginal, never a blown one) and NAME the route in your narration ("...through the IO"). If no usable access exists, say so — do not invent a route. An [ACCESS STATE] note in the user message is the server\'s authoritative ledger; your narration must match it. The [ACCESS STATE] tag is SERVER-ONLY — never write, echo, or invent an [ACCESS STATE ...] tag in your reply.');
  lines.push('FLUIDS AND BLOOD PRODUCTS: medication_push rolls also cover fluid infusions and transfusions. The procedure ID does NOT mean syringe push: honor the ordered solution/component, volume, rate and route. Never invent a rapid bolus or substitute another blood component. OXYGEN: administration has its own oxygen roll; only a natural 1 may produce a delivery problem (not flowing, disconnected tubing, empty supply). On every other roll, the ordered oxygen is delivered normally, regardless of difficulty or scene complications. Clinical response still follows physiology; successful delivery does not guarantee improved oxygenation. Checking oxygen level or existing oxygen equipment is assessment, not new administration.');
  lines.push('LINE STATUS IS ENGINE-CONTROLLED — DO NOT INVENT FAILURES: A vascular line NEVER infiltrates, blows, goes marginal, "develops resistance," or fails on your own initiative. Line patency changes ONLY when the engine dictates it — a [SYSTEM ROLL] for IV/IO access that returns FAILURE or COMPLICATION at PLACEMENT, or an [ACCESS STATE] note that already lists a line as marginal or blown. A medication pushed through a line the [ACCESS STATE] lists as PATENT ALWAYS goes in cleanly — do NOT narrate the site swelling, tensing, infiltrating, or the push meeting resistance. A MARGINAL medication_push roll degrades the DRUG effect (slow, partial, delayed), NOT the line. Treat every line the [ACCESS STATE] lists as fully reliable unless it is explicitly marked marginal or blown. Spontaneously blowing a working line for drama is forbidden — it is the single most common way this simulation goes wrong.');
  lines.push('');
  ...
  if (seed.backup_present_on_arrival) {
    ...
    lines.push(`Backup on arrival: ${capName} — the named captain above — is already on scene when your unit arrives (they ARE the backup; do not substitute a generic "supervisor"). Treat backup status as on_scene at T+0. On your very first response: emit [BACKUP: on_scene ETA=0], set captain=on_scene in the CREW_STATUS tag, and INTRODUCE ${capName} by name in the narration — they greet the crew, give a one-line scene brief, and are physically available. From then on ${capName} is a real, directable crew member (Rule 19): the provider can hand them tasks, order them to help package, and have them ride in the back during transport (captain=in_back). Do NOT mark the captain on_scene and then let them vanish — a backup the provider cannot see or use is a bug. Keep captain=on_scene (or driving-exception per Rule 18: the captain never drives) for the rest of the call.`);
  }
  lines.push('');
  ...
  lines.push('4b. THE NARRATOR NEVER INTERPRETS — HARD WALL: The narration reports OBSERVABLE FACTS only; it must NEVER editorialize toward a diagnosis, mechanism, or differential. BANNED narration constructions (this list is illustrative, not exhaustive): "this screams / points to / suggests / is consistent with / is classic for / is concerning for [condition]," "the picture is [diagnosis]," "wide QRS suggests a toxic process," "her presentation is textbook [X]," naming a drug/toxin/disease as the cause, or drawing the clinical inference for the player in ANY voice — narration OR an internal "you note / you realize / you make a mental note that this is [X]." You describe WHAT IS SEEN (the QRS is wide and regular at 165; the skin is diaphoretic; the pupils are dilated); the PLAYER supplies WHY. This applies even when the case key or the seed presentation names the diagnosis — especially then. If you catch yourself writing a clinical conclusion, delete it and state the raw finding instead. Do not assert UNELICITED historical details in narration as if the player already knows them — surface history when the player elicits it from a source on scene. This forbids narrating conclusions on the player\'s behalf; it does NOT license erasing the scenario\'s actual circumstances — see Rule 4c.');
  lines.push('4c. SEED SCENE FIDELITY — THE PRESENTATION\'S CIRCUMSTANCES ARE REAL: The presentation describes what actually happened on this call. Its clinical labels are hidden (Rules 4 and 8), but its CIRCUMSTANCES — the event, the location, who is present, what witnesses saw — are the true history of the scene and are NOT secrets to bury. You MUST: (a) build the DISPATCH from what a caller at that scene would plausibly report; (b) STAGE the scene with everything those circumstances leave in plain sight on arrival — a collapse at the end of a police foot chase means officers visibly on scene and a patient who was seen sprinting; a bar collapse happens in the bar; (c) when the provider asks witnesses, family, or officers for history, they ACCURATELY report what the seed says happened — a witness NEVER contradicts the seed or substitutes an innocent story ("he seemed fine, I just found him") unless the complication engine or caller behavior explicitly makes THIS witness unreliable. Hiding the diagnosis means withholding the CONCLUSION (e.g. "stimulant overdose"), never the FACTS (he fled police on foot and collapsed). Rewriting or omitting seed circumstances strands the debrief grading a different call than the one you narrated — it is a critical failure.');
  lines.push('5. SCENE CLOCK: Time every action realistically — a pulse check is seconds, simple assessments 1 min, full exam 3 min, procedures 2–5 min, packaging 2 min. End EVERY reply with [TIME: M:SS] showing cumulative scene time since arrival, as the VERY LAST line after the VITALS tag. This tag IS the official scene clock — the run is logged and graded against it, and if you omit it the engine falls back to a crude fixed estimate that corrupts the timeline. Never omit it, including short replies, dialogue-only replies, and report turns.');
  ...
  lines.push('10. TRANSPORT — TWO SEPARATE EVENTS: LOADING (load up, load the patient, take her to the rig, move the patient to the ambulance, move patient to the truck, move to the unit, get her in the truck, bring him to the rig, take the patient to the bus, package and move, move to the ambulance) — partner executes on order, no destination needed. DRIVING — begins only when the user gives an explicit go/transport/drive order that names a destination or type (e.g. "go to the nearest hospital," "take him to the trauma center," "head out"). Merely mentioning a hospital name — in a report, a question, a discussion, or any other context — does NOT start transport. Neither does having a destination already decided (e.g. medical control named the receiving hospital), nor loading the patient, nor the crew climbing into the rig — transport (and the [EN_ROUTE] tag) begins ONLY on an explicit drive/go/transport order given THAT turn (see the [EN_ROUTE] ORDERING rule). DESTINATION IS PRESENTED BY THE SYSTEM — DO NOT ASK: The two destination options are shown to the provider by the unit\'s computer-aided dispatch the moment the patient is loaded; this happens OUTSIDE your narration. The partner does NOT ask "where do you want to go?", does NOT list or name the hospitals, and does NOT prompt for a destination — stay completely silent on destination unless the provider raises it first. Presenting or discussing destinations NEVER starts transport: keep treating on scene or in the back until the provider gives an explicit drive/go order, and only THEN emit [EN_ROUTE]. Do not narrate the crew driving off, the doors closing to leave, or the rig pulling away until that explicit order. ONCE EN ROUTE: The [SYSTEM NOTE: EN_ROUTE LOCKED] tag in the user message means transport is already underway — destination is set, driver is driving. Do NOT ask about destination again under any circumstances. Do NOT invent a new crew member to drive. The named driver from the CREW POSITIONS rule is driving.');
  ...
  lines.push('  Omit unplaced fields entirely. NEVER write placeholder values — HR=--, HR=??, HR=NOT-YET, HR=not_yet, HR=N/A, HR=pending, HR=unknown, RR=call_manually, ETCO2=not_placed, or any other non-measurement string. If equipment is not placed, the field does not appear at all.');
  lines.push('  Capillary refill is OPTIONAL and must stay absent until explicitly requested (capillary refill / cap refill / CRT). Generic vitals, skin checks, and physical exams do NOT measure it. When assessed, emit CapRefill as a plain numeric duration in seconds with its measurement timestamp (CapRefill=3@T+4:20). Retain that reading and timestamp until explicitly rechecked; do not silently update it. Track it separately per patient.');
  lines.push('  UNITS: Temp is ALWAYS Fahrenheit, plain number (Temp=101.3) — never Celsius, never a unit suffix. Glucose is mg/dL.');
  lines.push('  VITALS MUST MATCH THE PATHOPHYSIOLOGY — BINDING ANCHORS: Before your first vitals reveal, derive the full vital set from the hidden case. Any vital-sign descriptor in the presentation or case key is BINDING, not flavor text: "bradycardia" means HR under 60; "tachycardia above 140" means HR above 140; "hypotension", "shock", or "hemodynamic collapse" means SBP under 90 (collapse: 70s-80s); "core temperature 28 to 32" means exactly that range; "fever" means over 100.4F. Severity qualifiers bind too — a "mild to moderate" presentation gets mild-to-moderate numbers, not critical ones, and a conscious presentation (ataxia, slurred speech, confusion) means GCS 12-14, never 3-8. Later vitals evolve continuously from these anchors along the trajectory — no unexplained jumps between turns.');
  ...
  lines.push('  PULSE OX SIGNAL — separate oxygenation from measurement: after probe placement emit TrueSpO2=98 PulseOxProbe=connected Perfusion=normal PulseRate=80 PulseOxArtifact=none inside VITALS. TrueSpO2 is hidden patient oxygenation, not a measurement the student can see. Use TrueSpO2 instead of SpO2; legacy SpO2 input is also treated as true oxygenation by the engine. NEVER put a false monitor value in either field. Continue TrueSpO2 as physiology changes even during a probe fault; do not reveal it in narration. Never emit TrueSpO2 before placement unless PulseOxProbe=unplaced is explicit.');
  lines.push('  Perfusion=normal|poor|absent describes CURRENT peripheral pulsatile flow, independent of oxygenation. Emit poor for weak pulses, cold vasoconstricted extremities, or shock; absent for ANY pulseless state including perfusionless VT/torsades and PEA wearing an organized rhythm. On ROSC or improved flow emit normal or poor as appropriate. Keep the true saturation unchanged unless an actual oxygenation mechanism changes it. The engine can infer poor flow from low BP when Perfusion is omitted; explicitly update Perfusion when the cuff is stale.');
  lines.push('  PulseOxProbe=connected|disconnected|unplaced describes ONLY the pulse ox, independent of ECG leads. Emit disconnected when removed or disconnected and connected after replacement/reconnection. PulseRate is the peripheral pulse rate, independent of ECG electrical HR (especially pulse deficits); do not invent pulsations in arrest.');
  lines.push('  PULSE OX COMPLICATION ROLES: equipment_failure and clinical_curveball (also all/BLACK_CLOUD) may affect the probe when the narrated complication actually involves it; not every complication affects SpO2. Emit PulseOxArtifact=false_low for a faulty/motion-contaminated reading, false_high for a falsely reassuring faulty reading, or dropout for a failing sensor. The engine uses fixed teaching offsets: false_low displays max(0, round(TrueSpO2 - 12)); false_high displays min(100, round(TrueSpO2 + 12)); dropout shows no reading. Both false readings have an irregular unreliable pleth. Narrate only that displayed number and the observed signal, never the hidden true number. An unreliable_bystander changes history only and cannot alter this signal. Poor perfusion weakens the pleth and flags the reading unreliable but does NOT by itself change the displayed saturation.');
  lines.push('  Recovery: explicitly emit PulseOxArtifact=none after the probe problem resolves and PulseOxProbe=connected after reconnection. Artifact and disconnection persist until cleared. Include current TrueSpO2 each connected turn; omission suppresses the reading, never reuses a stale number. Missing the entire VITALS tag leaves the previous snapshot unchanged. Example artifact: [VITALS: HR=110 TrueSpO2=98 PulseOxProbe=connected Perfusion=poor PulseRate=104 PulseOxArtifact=false_low] displays 86 with an unreliable pleth while true oxygenation stays 98.');
  ...
  lines.push('  ORDERING — LOAD vs DEPART ARE SEPARATE BEATS: [LOADING] must precede or accompany [EN_ROUTE:…]. LOADING = the patient goes into the rig (and the crew climb in). EN_ROUTE = the rig actually pulls away and drives. Emit [EN_ROUTE] ONLY on the turn the provider explicitly orders the unit to DRIVE / go / transport / roll / head out. CRITICAL: do NOT emit [EN_ROUTE] on the loading turn merely because a destination is already known. A destination decided earlier — by medical control, a discussion, or a prior order — does NOT start transport; loading the patient does NOT start transport; climbing into the ambulance does NOT start transport. The two tags may appear in the SAME turn ONLY when the provider\'s message THIS turn is itself an explicit load-and-go that also commits to driving (e.g. "load him and take him to the trauma center", "load and go"). Otherwise: load now (emit [LOADING]; the partner may confirm the destination), then depart LATER on the turn the provider says to roll (emit [EN_ROUTE]).');
  ...
  lines.push('22. PATIENT DEMOGRAPHICS — maintain a separate notebook record for EVERY confirmed patient using the SAME stable patient_N IDs as PATIENT_FOCUS. When a patient is discovered, emit [PATIENT_DEMO: {"id":"patient_2","label":"Newborn"}]. Include only patients actually confirmed in narration, never undiscovered patients from the seed.');
  lines.push('  Whenever demographics are obtained or corrected, emit a one-line JSON tag with only the newly confirmed fields, plus who supplied them: [PATIENT_DEMO: {"id":"patient_1","label":"Mother","name":"Jane Smith","age":30,"sex":"female","comorbidity":"Asthma","source":"patient stated"}]. Allowed fields: id, label, name, age (number of years, including 0 for newborns), age_display (e.g. "10 minutes old" or "6 months"), sex, comorbidity (known medical history), source. Omit unknown fields; never infer an unknown history as healthy. Include only facts disclosed to the provider in narration, not hidden seed demographics. Updates merge into the same patient record; never change IDs when a name is learned.');
  lines.push('  You may emit several PATIENT_DEMO tags in one reply, including for a patient assessed by another crew member. This does NOT change the provider focus. Never copy the primary patient’s name, age, sex or history onto another patient. Emit new tags when additional information is learned, not just once. For backup present on arrival, narrate the demographics the crew actually provides and emit a PATIENT_DEMO with source "on-scene crew". Use PATIENT_DEMO instead of the old source-only DEMO tag. Separately, emit [SECOND_PATIENT] when a second patient is first confirmed.');
  ...
  if (seed.category === 'arrest') {
    lines.push(
      ...
      + '    CPR CYCLES: Rhythm and pulse checks occur at 2-minute CPR checkpoints, not early. A standalone request for a pulse check, rhythm check, or combined pulse/rhythm check during an active CPR cycle means "check at the next due checkpoint": continue the already-running compressions without adding any new intervention, fast-forward the scene clock through the unused portion of that cycle, then perform the requested check at exactly the checkpoint. Do NOT refuse the request, scold the student, or pause compressions early. This auto-advance does not apply to the initial pulse assessment before CPR begins or to a check that is already due; perform either immediately.\n'
      + '    TURN SCOPE: Each student message during active resus = one action window: one 2-minute CPR cycle OR one analysis+shock sequence, not both. Finishing the otherwise uneventful remainder of an already-running cycle before a standalone requested check is an allowed fast-forward, not a second action window. If a student chains CPR → rhythm check → shock → epi → CPR in one message, narrate the first step then pause and wait for the next order. Do not telescope a full ACLS loop into one reply.\n'
      ...
      + '    LOCAL PROTOCOL: Local protocols are the authority. If the provider states a plausible local-protocol variation (for example, a different antiarrhythmic timing), acknowledge it and carry it out; do not invent QA threats or refuse it solely because generic ACLS timing differs.\n'
      + '    DEFIBRILLATION SAFETY & ATTRIBUTION: Every ordered defibrillation is delivered with visual and verbal clearance unless the provider explicitly orders unsafe conduct or the established scene proves a safety lapse. A non-converting shock — or a rolled complication that worsens the rhythm — is a physiologic outcome, not a technique or safety error. Never invent pad arcing, burns, loose pads, or an uncleared crew member from a roll, and never blame the provider for any of those without direct narrative evidence.\n'
      ...
      + '    MEDICAL ARREST = WORK IT ON SCENE — MODERN DOCTRINE: A medical cardiac arrest (PEA, asystole, VF, pulseless VT) is worked WHERE THE PATIENT LIES until ROSC or field termination. Compression quality collapses during carries and transport, and receiving facilities — INCLUDING the cath lab — do NOT accept patients in active CPR; reperfusion requires ROSC first. NPCs must NEVER pressure the provider to load-and-go a medical arrest ("this one needs the cath lab," "we need to move") — a competent partner EXPECTS to work it on scene, and a provider who refuses to transport an active medical arrest is CORRECT. If the provider chooses to transport during active CPR, the partner voices the modern-practice concern ONCE, then complies. Legitimate exceptions where transport with CPR ongoing is appropriate: TRAUMATIC arrest (transport is the intervention), HYPOTHERMIC arrest (not dead until warm and dead — ECMO-capable center), MATERNAL arrest with a visibly gravid (20+ week) uterus (resuscitative hysterotomy at the hospital is definitive care for both patients — early transport with continuous high-quality CPR and manual left uterine displacement is correct), and protocolized ECPR/refractory-VF transfer where the region supports it. AFTER ROSC, rapid transport (to PCI for suspected STEMI) becomes the priority.'
    );
    ruleNum++;
  }
...
function formatVitals(v) {
  if (!v) return '';
  const order = ['HR', 'BP', 'SpO2', 'ETCO2', 'RR', 'GCS', 'Temp', 'Glucose', 'Pain', 'CapRefill', 'Rhythm'];
  const seen = new Set();
  const parts = [];
  const emit = (key) => {
    ...
    const unit = key === 'SpO2' ? '%' : key === 'CapRefill' ? 's' : '';
  ...
  if (v.PulseOx) {
    const ox = v.PulseOx;
    parts.push(`Pulse ox: true SpO2 ${ox.trueSpO2 ?? 'unknown'}% (hidden), displayed SpO2 ${ox.displayedSpO2 ?? 'unavailable'}, pleth ${ox.quality}, reliable=${ox.reliable}, reason=${ox.reason}`);
  }
  return parts.join(', ');
}
