...
function assembleSeedBlock(seed) {
  ...
  // --- Crew ---
  lines.push('--- CREW ---');
  lines.push('Both partner and captain are assigned to this unit. They are on scene with you from T+0 — not responding separately, not self-dispatching, not optional. Treat them as present and active throughout the call.');
  if (partner) {
  ...
  lines.push('19. BACKUP STATUS — MACHINE TAG: Track not_called → called → en_route → on_scene. Emit [BACKUP: STATUS] only when status changes. en_route and on_scene append ETA=N. called has no ETA. AUTO-BACKUP: cardiac arrest → [BACKUP: en_route ETA=8] on first response; multi-patient → [BACKUP: en_route ETA=6]; "Backup on arrival" seed → [BACKUP: on_scene ETA=0]. All other scenarios: only on student request. BACKUP CREW NAMING: When backup arrives, give the crew specific names — dispatch announces them by name (e.g. "Engine 12 on scene — Captain Rivera and EMT Sato"). Make names fit the region and unit type. Do not leave backup crew anonymous.');
