...
function assembleSeedBlock(seed) {
  ...
  lines.push('  WIDE-COMPLEX TACHYCARDIA — USE Rhythm=VT: Any wide-complex tachycardia — true VT, SVT with aberrancy, a paced tachycardia, or the QRS widening of sodium-channel toxicity (cocaine, TCA) — is emitted as Rhythm=VT so the monitor draws WIDE complexes. There is NO separate wide-complex token: NEVER invent one (e.g. wide_complex_tach, WCT, VTach) — an unrecognized token falls back to a NARROW sinus trace, which will contradict your narration. If the QRS is wide, the token is VT (or paced, if pacer-driven). Emit the token from the FIRST reading so the drawn waveform matches what you describe; do not narrate "wide" while the monitor shows narrow.');
