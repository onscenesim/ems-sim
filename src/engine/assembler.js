...
function assembleSeedBlock(seed) {
  ...
  lines.push('  ARREST PHYSIOLOGY — arrest is loss of a PULSE, not always loss of a monitor rhythm. Vary it realistically: (a) ASYSTOLE — flatline, HR=0, Rhythm=asystole; obvious. (b) VF / pulseless VT — Rhythm=VF or VT with an erratic or absent HR; alarming, recognized fast. (c) PEA — the common real-world picture: the monitor still shows an ORGANIZED rhythm and a REAL HR number (a slow sinus, junctional, or idioventricular at ~20-60, sometimes a near-normal rate) but there is NO pulse. In PEA keep emitting that HR and Rhythm — do NOT drop HR to 0 and do NOT omit HR. UNRECOGNIZED ARREST is a deliberate teaching opportunity: do NOT announce "the patient is in cardiac arrest." Reveal it only through findings the student must look for — absent pulse (only if they palpate), unobtainable BP, a sharp ETCO2 fall (dropping below ~20 toward 10), unresponsiveness, and apnea or agonal breathing. If the student keeps treating the HR number and never checks a pulse, let the arrest go unrecognized — that is the lesson.');
  ...
  lines.push('  [LOADING] — emit when the stretcher enters the ambulance and the patient is secured inside. Trigger phrases: "load the patient", "move to ambulance", "move patient to the ambulance", "take her to the rig", "load up", "get her in the unit", "bring him to the truck", "package and move" — any order to place the patient inside the vehicle. NOT for packaging on scene, NOT for simply walking toward the rig.');
