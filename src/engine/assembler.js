...
function assembleSeedBlock(seed) {
  ...
  lines.push('5. SCENE CLOCK + MANDATORY TIME TAG: Track simulated scene time in minutes. Every action takes time: simple assessments 1 min, full head-to-toe 3 min, procedures 2–5 min, packaging 2 min. On EVERY turn without exception, emit a [TIME: M:SS] tag at the end of your response showing the current cumulative scene time in minutes and seconds. Example: [TIME: 4:30] means 4 minutes 30 seconds elapsed since arrival. This tag is the authoritative scene clock parsed by the server — it MUST reflect the actual time elapsed based on actions taken, not an estimate. Do NOT omit this tag under any circumstances, including report turns, short replies, or NIBP cycles.');;
  ...
  lines.push('');
  lines.push('17. EVENT TAGS — MACHINE SIGNALS: In addition to the VITALS tag, you must emit each of these tags exactly once, when the described event first occurs. Do not emit a tag more than once per scenario.\n  [LOADING] — emit on the turn when the patient is physically loaded into the ambulance. This means the stretcher has entered the vehicle. Do NOT emit for packaging, moving toward the stretcher, or walking to the truck.\n  [EN_ROUTE] — emit on the turn when the unit actually begins driving toward the hospital. Do NOT emit when transport is ordered but not yet started — only when wheels are rolling.\n  ORDERING RULE: [LOADING] MUST always precede or accompany [EN_ROUTE]. If the student orders a destination (e.g. "go to General Hospital") without having loaded the patient yet, you MUST first narrate loading AND emit [LOADING], THEN narrate beginning to drive AND emit [EN_ROUTE]. Both tags may appear in the same turn if loading and driving happen back-to-back. NEVER emit [EN_ROUTE] without [LOADING] having been emitted in the same or a prior turn.');
  lines.push('');
