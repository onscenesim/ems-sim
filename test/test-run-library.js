...
test('library, details, transcripts, session export and replay enforce account ownership on every request', async () => {
  ...
  assert.equal(debrief.body.learning.debriefText, 'Test debrief.');
  assert.equal(debrief.body.learning.patientOutcome, undefined);
  assert.equal(persistence.load(saved.id).patientOutcome, 'Discharged home on September 26, 2026');
  const liveExport = await route('/:id/transcript', { params: { id: saved.id }, cookie: cookieA });
  assert.equal(liveExport.body.patientOutcome, 'Discharged home on September 26, 2026');
  assert.equal(liveExport.body.learningReview.debriefText, 'Test debrief.');
  assert.equal(liveExport.body.learningReview.patientOutcome, undefined);
  assert.ok(Array.isArray(liveExport.body.learningReview.timeline));
  ...
  assert.equal(detail.body.learning.debriefText, 'Saved debrief.');
  const savedExport = await route('/runs/:runId/transcript', { params: { runId: runA.id }, cookie: cookieA });
  assert.equal(savedExport.body.learning.debriefText, 'Saved debrief.');
  assert.equal(savedExport.body.learning.patientOutcome, undefined);
  assert.equal(savedExport.body.learning.timeline[0].scene, 'Visible assessment.');
